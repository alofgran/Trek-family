 ...
 * sets the httpOnly trek_family_session cookie from a valid auth code.
 */
...
describe('OIDC e2e (real cookie service)', () => {
  ...
  it('GET /exchange sets the httpOnly trek_family_session cookie + returns the token', async () => {
    oidcSvc.consumeAuthCode.mockReturnValue({ token: 'jwt.value' });
