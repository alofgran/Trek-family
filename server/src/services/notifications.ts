...
import { EMAIL_I18N as I18N, EVENT_TEXTS, PASSWORD_RESET_I18N } from '@trek-family/shared/i18n/externalNotifications';
import type { EmailStrings, EventText, PasswordResetStrings, NotificationEventKey } from '@trek-family/shared/i18n/externalNotifications';
...
// ── Email i18n strings — imported from @trek-family/shared/i18n/externalNotifications ──

// EVENT_TEXTS imported from @trek-family/shared/i18n/externalNotifications
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="background: linear-gradient(135deg, #000000 0%, #1a1a2e 100%); padding: 32px 32px 28px; text-align: center;">
          ...
          <div style="color: #ffffff; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">TREK FAMILY</div>
...
// PASSWORD_RESET_I18N imported from @trek-family/shared/i18n/externalNotifications
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `TREK FAMILY — ${strings.subject}`,
...
export function buildWebhookBody(url: string, payload: { event: string; title: string; body: string; tripName?: string; link?: string }): string {
  ...
  if (isDiscord) {
    return JSON.stringify({
      embeds: [{
        ...
        url: payload.link,
        color: 0x3b82f6,
        footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'TREK FAMILY' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in TREK FAMILY>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'TREK FAMILY' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'TREK FAMILY — Test Notification',
      text: 'This is a test email from TREK FAMILY. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, { event: 'test', title: 'Test Notification', body: 'This is a test webhook from TREK FAMILY. If you received this, your webhook configuration is working correctly.' });
...
export async function testNtfy(cfg: { topic: string; server?: string | null; token?: string | null }): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      ...
      body: 'This is a test notification from TREK FAMILY. If you received this, your ntfy configuration is working correctly.',
    });
